...
Z_DEGREE_RE  = re.compile(
    r"\b(b\.?tech|m\.?tech|b\.?e|m\.?e|b\.?sc|m\.?sc|b\.?a|m\.?a|"
    r"phd|mba|bca|mca|bachelor|master|doctorate)\b", re.IGNORECASE
)
...
def extract_education(section_text):
    ...
    for line in lines:
        ...
        if degree_match and current_lines:
            prev_text = "\n".join(current_lines)
            if DEGREE_RE.search(prev_text):
                entry = parse_education_block(prev_text)
                if entry.get("degree") or entry.get("institution"):
                    entries.append(entry)
    ...
    if not entries:
        for line in lines:
            ...
            if not stripped:
                continue
            if DEGREE_RE.search(stripped) or INSTITUTION_INDICATORS.search(stripped):
                entry = parse_education_block(stripped)
                if entry.get("degree") or entry.get("institution"):
                    entries.append(entry)
    ...
    if degree_match:
        ...
        if degree_line:
            ...
            if field_match:
                ...
                field = field.split("\n")[0].strip()
                # Must be a real field name, not institution/city
                if 3 < len(field) < 50 and not YEAR_RE.search(field) and not NON_FIELD_WORDS.search(field):
                    entry["field"] = field

    # Extract institution — look for line with institution indicator
    for line in block_lines:
        if INSTITUTION_INDICATORS.search(line):
            ...
            inst_text = YEAR_RE.sub("", inst_text)
            inst_text = GPA_RE.sub("", inst_text)
